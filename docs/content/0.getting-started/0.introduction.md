...
- Easily generate site-wide breadcrumb using the [`useBreadcrumbItems()`{lang="ts"}](/docs/seo-utils/api/breadcrumbs) composable (test).
...
- [Inline Minification](/docs/seo-utils/guides/minification): Automatically minify inline scripts (esbuild) and styles (lightningcss) at build time, with a lightweight runtime fallback for dynamic SSR
