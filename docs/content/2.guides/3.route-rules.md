...
Route rule `seoMeta` uses the module's configured `tagPriority` (default `30`). You can override this in your page components by setting a lower number (higher priority):
...
useSeoMeta({
  ...
  tagPriority: 10 // lower number = higher priority, overrides route rules
})
</script>
```
