...
relatedPages:
  - path: /docs/seo-utils/api/fallback-title
...
to make it easier: fallback titles, page meta titles, and a default title template.
...
**Error pages:** on error pages (e.g. 404), the title template reverts to just `'%s'` with no separator or site name appended. This avoids generating titles like `404 - Page not found | My Site`.
...
- `%separator`{lang="ts"} see [Title template params](/learn-seo/nuxt/mastering-meta/titles#template-params)
- `%siteName`{lang="ts"} see [Site Config](/docs/site-config/guides/how-it-works).
