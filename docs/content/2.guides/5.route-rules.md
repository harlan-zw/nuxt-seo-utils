...
description: Utilize route rules for dynamic SEO meta tags.
relatedPages:
  - path: /docs/seo-utils/guides/nuxt-config-seo-meta
...
Takes the same input as [useHead()](https://nuxt.com/docs/api/composables/use-head){lang="ts"}.
...
**Limitations:** route rule `seoMeta` uses the module's configured `tagPriority` (default `'low'`). Page-level `useSeoMeta()` calls automatically take precedence since they use the default priority.
