...
- **JavaScript**: Minified with [esbuild](https://esbuild.github.io/) (variable shortening, dead code elimination, whitespace removal)
...
export default defineNuxtConfig({
  seo: {
    // Enable both build and runtime (default)
    minify: true,

    // Disable entirely
    minify: false,

    // Toggle each mode independently
    minify: {
      build: true,    // esbuild + lightningcss at build time
      runtime: false,  // disable per-request minification
    },
  },
})
```
...
|---|---|---|
| `app.head` scripts/styles | esbuild / lightningcss | n/a (static) |
| Prerendered route HTML | esbuild / lightningcss | n/a (static) |
...
The following are never minified:

- Non-JavaScript script types: `application/ld+json`, `application/json`, `importmap`, `speculationrules`
- External scripts (with `src` attribute)
- Very small inline content (under 20 characters)
